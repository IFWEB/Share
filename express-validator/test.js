...
// console.log(validationCheck('username'));
//静态资源
...
app.all('/test1', [
    ...
    // 通常错误信息可以作为第二个参数
    validationCheck('password', 'passwords must be at least 5 chars long and contain one number')
    .isLength({ min: 5 })
], (req, res, next) => {
...
//加入自定义校验
app.all('/test2', [
    validationCheck('username', 'must be an email')
    .isEmail()
    .custom(value => {
        //自定义校验，个人自己写
        function vali(val) {
...
app.all('/test3', [
    validationCheck('username', 'username must be an email')
    .isEmail()
], (req, res, next) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(422).json({
          code: 1000,
          message: errors.array()[0].msg,//或者errors.mapped().username.msg
          data: {}
        });
    } else {
        return res.json({
